...
sub update_pae_attributes {
  ...
  if ($pae_control and $pae_control eq 'enabled') {
    ...
    schema('netdisco')->resultset('Device')->search({'me.ip' => $device->ip})->update({pae_is_enabled => 't'});
  }
  else {
    debug sprintf ' [%s] pae - no PortAccessEntity support, leaving worker', $device->ip;
    schema('netdisco')->resultset('Device')->search({'me.ip' => $device->ip})->update({pae_is_enabled => 'f'});
  ...
  foreach my $ind (sort keys %$interfaces) {
    debug sprintf ' [%s] pae - attributes found for ifindex %s: %s %s %s %s %s %s %s', $device->ip, $ind,
      $pae_authconfig_state->{$ind} || 'no pae_authconfig_state', $pae_authconfig_port_control->{$ind},
      $pae_authconfig_port_status->{$ind}, $pae_authsess_user->{$ind}, $pae_authsess_mab->{$ind},
      $pae_capabilities->{$ind}, $pae_last_eapol_frame_source->{$ind};

    # 更新设备端口属性
    schema('netdisco')
      ->resultset('DevicePortProperties')
