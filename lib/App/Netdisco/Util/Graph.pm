...
sub graph_each {
  ...
  # 边定义
  my $edge_defs = {'color' => $CONFIG{edge_color} || 'wheat',};
  ...
  # 设置根设备
  my $root_ip
    = defined $CONFIG{root_device} ? (ipv4_from_hostname($CONFIG{root_device}) || $CONFIG{root_device}) : undef;
...
sub make_graph {
  ...
  # 获取设备和链接信息
  my $devices = schema(vars->{'tenant'})->resultset('Device')->search({}, {columns => [qw/ip dns location /]});
  my $links
    = schema(vars->{'tenant'})
    ->resultset('DevicePort')
    ->search({remote_ip => {-not => undef}}, {columns => [qw/ip remote_ip speed remote_type/]});
  my %aliases = map { $_->alias => $_->ip }
    schema(vars->{'tenant'})->resultset('DeviceIp')->search({}, {columns => [qw/ip alias/]})->all;
    ...
    foreach my $dest (@{$linkmap{$link}}) {
      ...
      foreach my $side ($link, $dest) {
        unless (defined $GRAPH{$side}) {
          ...
          my $dns    = $is_dev ? $devs{$side} : hostname_from_ip($side);
