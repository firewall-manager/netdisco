...
sub request_is_device {
  return (
      ...
      or index(request->path, uri_for('/ajax/content/device/details')->path) == 0
      or index(request->path, uri_for('/ajax/content/device/ports')->path) == 0
  );
}
...
sub request_is_api {
  return (
    (request->accept and request->accept =~ m/(?:json|javascript)/)
      ...
      or (param('return_url') and index(param('return_url'), uri_for('/api/')->path) == 0))
  );
}
...
sub request_is_api_report {
  return (
    request_is_api
      and (index(request->path, uri_for('/api/v1/report/')->path) == 0
      or (param('return_url') and index(param('return_url'), uri_for('/api/v1/report/')->path) == 0))
  );
}
...
sub request_is_api_search {
  return (
    request_is_api
      and (index(request->path, uri_for('/api/v1/search/')->path) == 0
      or (param('return_url') and index(param('return_url'), uri_for('/api/v1/search/')->path) == 0))
  );
}
...
sub interval_to_daterange {
  ...
  unless ($interval and $interval =~ m/^(?:\d+)\s+(?:day|week|month|year)s?$/) {

    return "1970-01-01 to " . Time::Piece->new->ymd;
  }

  my %const = (day => ONE_DAY, week => ONE_WEEK, month => ONE_MONTH, year => ONE_YEAR);

  my ($amt, $factor) = $interval =~ /^(\d+)\s+(day|week|month|year)s?$/gmx;
