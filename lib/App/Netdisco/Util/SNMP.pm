...
sub get_communities {
  ...
  eval { $device->community->update({$tag_name => undef}) }
    if $device->in_storage and (not $stored_tag or !exists $seen_tags->{$stored_tag});

  # 确保所有标记的都在传统团体之前
  push @communities, @$config;

  # 尝试最后已知良好的v2读取
  push @communities, {read => 1, write => 0, driver => 'snmp', only => [$device->ip], community => $device->snmp_comm,}
    if defined $device->snmp_comm and $mode eq 'read';

  # 尝试最后已知良好的v2写入
  my $snmp_comm_rw = eval { $device->community->snmp_comm_rw };
  push @communities, {write => 1, read => 0, driver => 'snmp', only => [$device->ip], community => $snmp_comm_rw,}
...
sub snmp_comm_reindex {
  ...
  if ($ver == 3) {
    ...
    # 查找用户配置的上下文前缀
    foreach my $c (@comms) {
      next unless $c->{tag} and $c->{tag} eq (eval { $device->community->snmp_auth_tag_read } || '');
      $prefix = $c->{context_prefix} and last;
    }
    $prefix ||= 'vlan-';

    if ($vlan =~ /^[0-9]+$/i && $vlan) {
      debug sprintf ' [%s] reindexing to "%s%s" (ver: %s, class: %s)', $device->ip, $prefix, $vlan, $ver, $snmp->class;
      $snmp->update(Context => ($prefix . $vlan));
    }
    elsif ($vlan =~ /^[a-z0-9]+$/i && $vlan) {
      debug sprintf ' [%s] reindexing to "%s" (ver: %s, class: %s)', $device->ip, $vlan, $ver, $snmp->class;
      $snmp->update(Context => ($vlan));
    }
    else {
      debug sprintf ' [%s] reindexing without context (ver: %s, class: %s)', $device->ip, $ver, $snmp->class;
      $snmp->update(Context => '');
    }
  }
  else {
    ...
    debug sprintf ' [%s] reindexing to vlan %s (ver: %s, class: %s)', $device->ip, $vlan, $ver, $snmp->class;
    $vlan ? $snmp->update(Community => $comm . '@' . $vlan) : $snmp->update(Community => $comm);
  }
...
sub get_mibdirs {
  ...
  return map { dir($home, $_)->stringify } @{setting('mibdirs') || get_mibdirs_shortnames()};
}
...
sub decode_and_munge {
  ...
  $data
    = (ref {} eq ref $data)
    ? {map { ($_ => (defined $data->{$_} ? decode_base64($data->{$_}) : undef)) } keys %$data}
  ...
  $data
    = (ref {} eq ref $data)
    ? {map { ($_ => (defined $data->{$_} ? $class->can($sub)->($data->{$_}) : undef)) } keys %$data}
