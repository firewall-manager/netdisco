...
sub txn_do_locked {
  ...
  $schema->throw_exception('missing Table name to txn_do_locked()') unless $table;
  ...
  if (ref '' eq ref $mode and $mode) {
    scalar grep { $_ eq $mode } values %lock_modes or $schema->throw_exception('bad LOCK_MODE to txn_do_locked()');
  }
  else {
    $sub  = $mode;
