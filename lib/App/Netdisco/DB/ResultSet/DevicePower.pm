  ...
  return $rs->search_rs($cond, $attrs)->search(
    {},
    {
      'columns' => {
        ...
        poe_powered_ports => \
          "SUM(CASE WHEN ports.status = 'deliveringPower' THEN 1 ELSE 0 END) OVER (PARTITION BY me.ip, me.module)",
        poe_disabled_ports => \
          "SUM(CASE WHEN ports.admin = 'false' THEN 1 ELSE 0 END) OVER (PARTITION BY me.ip, me.module)",
        poe_errored_ports => \
          "SUM(CASE WHEN ports.status ILIKE '%fault' THEN 1 ELSE 0 END) OVER (PARTITION BY me.ip, me.module)",
        poe_power_committed => \(
              "SUM(CASE "
        ...
        poe_power_delivering => \(
              "SUM(CASE WHEN (ports.power IS NULL OR ports.power = '0') "
