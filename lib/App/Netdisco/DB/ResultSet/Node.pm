...
sub delete {
  ...
  if (exists $opts->{archive_nodes} and $opts->{archive_nodes}) {
    foreach my $set (
      ...
      $schema->resultset($set)->search({mac => {'-in' => $nodes->as_query}},)->update({active => \'false'});
    }

    $schema->resultset('NodeWireless')->search({mac => {'-in' => $nodes->as_query}})->delete;
    ...
    foreach my $set (
      ...
      $schema->resultset($set)->search({mac => {'-in' => $nodes->as_query}},)->delete;
    }
