...
sub get_distinct_col {
  ...
  return $rs->search({}, {columns => [$col], order_by => $col, distinct => 1})->get_column($col)->all;
}
...
sub _with_datatables_order_clause {
  ...
  if (defined $params->{'order'}{0}) {
    for (my $i = 0; $i < (scalar keys %{$params->{'order'}}); $i++) {
      ...
      # 我们只得到列索引（从0开始），所以必须
      # 将索引转换为列名
      my $column_name = _datatables_index_to_column($params, $params->{'order'}{$i}{'column'});
...
sub _with_datatables_where_clause {
  ...
  if (defined $params->{'search'}{'value'} && $params->{'search'}{'value'}) {
    ...
    for (my $i = 0; $i < (scalar keys %{$params->{'columns'}}); $i++) {
      ...
      if ($params->{'columns'}{$i}{'searchable'} and $params->{'columns'}{$i}{'searchable'} eq 'true') {
