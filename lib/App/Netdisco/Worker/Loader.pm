...
sub load_workers {
  ...
  foreach my $phase (qw/check early main user store late/) {
    ...
    next unless exists vars->{'workers'}->{'internal'} and exists vars->{'workers'}->{'internal'}->{$phase};
    ...
    foreach my $namespace (sort keys %{$internal->{$phase}}) {
      foreach my $priority (keys %{$internal->{$phase}->{$namespace}}) {
        push @{$workers->{$phase}->{'__internal__'}->{$priority}}, @{$internal->{$phase}->{$namespace}->{$priority}};
      }
    }
  }
  ...
  foreach my $phase (qw/check early main user store late/) {
    ...
    foreach my $namespace (sort keys %{$workers->{$phase}}) {
      foreach my $priority (sort { $b <=> $a } keys %{$workers->{$phase}->{$namespace}}) {

        ++$driverless_main
          if $phase eq 'main' and ($priority == 0 or $priority == setting('driver_priority')->{'direct'});
