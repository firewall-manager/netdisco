...
register_worker(
  ...
  sub {
    ...
    # 检查调度器数据
    return Status->error("Missing data of Scheduler entry") unless $job->extra;
...
register_worker(
  ...
  sub {
    ...
    # 检查调度器标签
    return Status->error("Missing label of Scheduler entry") unless $action;

    # 获取需要调度的设备列表
    my @walk = schema(vars->{'tenant'})->resultset('Virtual::WalkJobs')->search(
      undef, {
        bind => [
          $action, ('scheduled-' . $sched->{label}),
          setting('workers')->{'max_deferrals'}, setting('workers')->{'retry_after'},
        ]
      }
    )->get_column('ip')->all;
