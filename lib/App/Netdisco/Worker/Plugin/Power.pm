...
register_worker(
  ...
  sub {
    ...
    # 检查设备参数
    return Status->error('Power failed: unable to interpret device param') unless defined $device;

    # 检查设备是否已发现
    return Status->error("Power skipped: $device not yet discovered") unless $device->in_storage;
    ...
    # 检查端口是否支持PoE
    return Status->error("No PoE service on port [$port] on device $device") unless vars->{'port'}->power;
...
register_worker(
  ...
  sub {
    ...
    if (!defined $rv) {
      return Status->error(sprintf 'failed to set [%s] power to [%s] on [%s]: %s', $pn, $data, $device,
        ($snmp->error || ''));
    }
    ...
    # 更新Netdisco数据库
    vars->{'port'}->power->update({admin => $data, status => ($data eq 'false' ? 'disabled' : 'searching'),});
