...
register_worker(
  {phase => 'main', driver => 'direct', title => 'gather macs from file'},
  sub {

    my ($job, $workerconf) = @_;
    my $device = $job->device;

    return Status->info('skip: fwtable data supplied by other source') unless $job->is_offline;

    # load cache from file or copy from job param
    my $data    = $job->extra;
    my @fwtable = (length $data ? @{from_json($data)} : ());

    return $job->cancel('data provided but 0 fwd entries found') unless scalar @fwtable;

    debug sprintf ' [%s] macsuck - %s forwarding table entries provided', $device->ip, scalar @fwtable;
    ...
    foreach my $node (@fwtable) {
      ...
      vars->{'fwtable'}->{$node->{'vlan'} || 0}->{$node->{'port'}}->{$mac->as_ieee} += 1;
    }
...
register_worker(
  {phase => 'main', driver => 'cli', title => 'gather macs from CLI'},
  sub {
    ...
    return $job->cancel('data provided but 0 fwd entries found') unless $nodecount;

    debug sprintf ' [%s] macsuck - %s forwarding table entries provided', $device->ip, $nodecount;
...
register_worker(
  {phase => 'main', driver => 'snmp', title => 'gather macs from snmp'},
  sub {
    ...
    my @vlan_list = get_vlan_list($snmp, $device);
    {
      ...
      foreach my $vlan (@vlan_list) {
        ...
        my $pv_fwtable = walk_fwtable($snmp, $device, $interfaces, $vlan);
...
register_worker(
  {phase => 'store', title => 'save macs to database'},
  sub {
    ...
    foreach my $vlan (keys %{vars->{'fwtable'}}) {
      delete vars->{'fwtable'}->{$vlan} unless scalar grep { $_ eq $vlan } @vlans;
    }
    ...
    foreach my $vlan (reverse sort keys %{vars->{'fwtable'}}) {
      foreach my $port (keys %{vars->{'fwtable'}->{$vlan}}) {
        ...
        debug sprintf ' [%s] macsuck - port %s vlan %s : %s nodes', $device->ip, $port, $vlabel,
          scalar keys %{vars->{'fwtable'}->{$vlan}->{$port}};

        foreach my $mac (keys %{vars->{'fwtable'}->{$vlan}->{$port}}) {

          # remove vlan 0 entry for this MAC addr
          delete vars->{'fwtable'}->{0}->{$_}->{$mac} for keys %{vars->{'fwtable'}->{0}};
    ...
    debug sprintf ' [%s] macsuck - stored %s forwarding table entries', $device->ip, $node_count;
    ...
    if (setting('node_freshness')) {
      $archived
        ...
        ->resultset('Node')
        ->search({
        switch => $device->ip, time_last => \["< ($now - ?::interval)", setting('node_freshness') . ' minutes'],
        })
        ->update({active => \'false'});
    }

    debug sprintf ' [%s] macsuck - removed %d fwd table entries to archive', $device->ip, $archived;
...
sub store_node {
  ...
  my $oui
    ...
    ->search({range => {'@>' => \[q{('x' || lpad( translate( ? ::text, ':', ''), 16, '0')) ::bit(64) ::bigint}, $mac]}},
    {rows => 1, columns => 'base'})
    ->first;
...
sub get_vlan_list {
  ...
  debug sprintf ' [%s] macsuck - VLANs: %s', $device->ip, (join ',', sort grep {$_} keys %vlans);
...
sub sanity_vlans {
  ...
  foreach my $vlan (sort keys %$vlans) {
    ...
    if (ref [] eq ref setting('macsuck_no_vlan')) {
      ...
      if ((scalar grep { $_ eq $vlan } @$ignore) or (scalar grep { $_ eq $name } @$ignore)) {

        debug sprintf ' [%s] macsuck VLAN %s - skipped by macsuck_no_vlan config', $device->ip, $vlan;
        next;
      }
    }
    ...
    if (ref [] eq ref setting('macsuck_no_devicevlan')) {
      ...
      if ((scalar grep { $_ eq "$ip:$vlan" } @$ignore) or (scalar grep { $_ eq "$ip:$name" } @$ignore)) {

        debug sprintf ' [%s] macsuck VLAN %s - skipped by macsuck_no_devicevlan config', $device->ip, $vlan;
        next;
      }
    }

    if (setting('macsuck_no_unnamed') and $name eq '(unnamed)') {
      debug sprintf ' [%s] macsuck VLAN %s - skipped by macsuck_no_unnamed config', $device->ip, $vlan;
      next;
    }

    if ($vlan > 4094) {
      debug sprintf ' [%s] macsuck - invalid VLAN number %s', $device->ip, $vlan;
      next;
    }
    ...
    if (not $vlans->{$vlan} and not setting('macsuck_all_vlans')) {
      debug sprintf ' [%s] macsuck VLAN %s/%s - not in use by any port - skipping.', $device->ip, $vlan, $name;
      next;
    }

    # check if vlan is in state 'suspended'
    if ($state eq 'suspended') {
      debug sprintf ' [%s] macsuck VLAN %s - VLAN is suspended - skipping.', $device->ip, $vlan;
      next;
    }
...
sub walk_fwtable {
  ...
  my $fw_vlan  = ($snmp->can('cisco_comm_indexing') and $snmp->cisco_comm_indexing()) ? {} : $snmp->qb_fw_vlan;
...
MAC: while (my ($idx, $mac) = each %$fw_mac) {
    ...
    unless (defined $bp_id) {
      debug sprintf ' [%s] macsuck %s - %s has no fw_port mapping - skipping.', $device->ip, $mac, $idx;
      next MAC;
    }
    ...
    unless (defined $iid) {
      debug sprintf ' [%s] macsuck %s - port %s has no bp_index mapping - skipping.', $device->ip, $mac, $bp_id;
      next MAC;
    }
    ...
    unless (defined $port) {
      debug sprintf ' [%s] macsuck %s - iid %s has no port mapping - skipping.', $device->ip, $mac, $iid;
      next MAC;
    }
    ...
    foreach my $map (@ignoremaps) {
      ...
      foreach my $key (sort keys %$map) {
        ...
        foreach my $port (sort { sort_port($a, $b) } keys %{$device_ports}) {
          ...
          debug sprintf ' [%s] macsuck %s - port suppressed by macsuck_no_deviceports', $device->ip, $port;
          ++$ignoreport->{$port};
        }
      }
    }
  ...
  foreach my $vlan (keys %{$cache}) {
    ...
    MAC: foreach my $mac (keys %{$cache->{$vlan}->{$port}}) {
        ...
        unless (defined $device_port) {
          debug sprintf ' [%s] macsuck %s - port %s is not in database - skipping.', $device->ip, $mac, $port;
          delete $cache->{$vlan}->{$port}->{$mac};
          next MAC;
        }

        if (exists $ignoreport->{$port}) {
          debug sprintf ' [%s] macsuck %s - port %s is suppressed by config - skipping.', $device->ip, $mac, $port;
          delete $cache->{$vlan}->{$port}->{$mac};
          next MAC;
        }

        if (exists $neighborport->{$port}) {
          debug sprintf ' [%s] macsuck %s - seen another device thru port %s - skipping.', $device->ip, $mac, $port;
          delete $cache->{$vlan}->{$port}->{$mac};
          next MAC;
        }
        ...
        if ($device_port->is_uplink) {
          if ($neigh_cannot_macsuck) {
            debug sprintf ' [%s] macsuck %s - port %s neighbor %s without macsuck support', $device->ip, $mac, $port,
              (eval { $device_port->neighbor->ip } || ($device_port->remote_ip || $device_port->remote_id || '?'));

            # continue!!
          }
          elsif (my $neighbor = $device_port->neighbor) {
            debug sprintf ' [%s] macsuck %s - port %s has neighbor %s - skipping.', $device->ip, $mac, $port,
              $neighbor->ip;
            delete $cache->{$vlan}->{$port}->{$mac};
            next MAC;
          }
          elsif (my $remote = $device_port->remote_ip) {
            debug sprintf ' [%s] macsuck %s - port %s has undiscovered neighbor %s', $device->ip, $mac, $port, $remote;

            # continue!!
          }
          elsif (not setting('macsuck_bleed')) {
            debug sprintf ' [%s] macsuck %s - port %s is detected uplink - skipping.', $device->ip, $mac, $port;
        ...
        if (exists $port_macs->{$mac}) {
          ...
          if ($device->ip eq $switch_ip) {
            debug sprintf ' [%s] macsuck %s - port %s connects to self - skipping.', $device->ip, $mac, $port;
            delete $cache->{$vlan}->{$port}->{$mac};
            next MAC;
          }

          debug sprintf ' [%s] macsuck %s - port %s is probably an uplink', $device->ip, $mac, $port;
          ...
          if (not setting('macsuck_bleed')) {
            debug sprintf ' [%s] macsuck %s - port %s is at topology edge', $device->ip, $mac, $port;
        ...
        if (defined $device_port->slave_of and exists $device_ports->{$device_port->slave_of}) {
