...
register_worker(
  ...
  sub {
    ...
    return Status->info(sprintf ' [%s] macsuck - dot11 info available but skipped due to config', $device->ip)
      unless setting('store_wireless_clients');
    ...
    while (my ($idx, $txrates) = each %$cd11_txrate) {
      ...
      my $txrate = (ref $txrates and defined $txrates->[$#$txrates]) ? int($txrates->[$#$txrates]) : undef;

      my $maxrate = (ref $rates and defined $rates->[$#$rates]) ? int($rates->[$#$rates]) : undef;

      my $ssid = $cd11_ssid->{$idx} || 'unknown';

      schema('netdisco')->txn_do(sub {
        schema('netdisco')
          ->resultset('NodeWireless')
          ...
          {order_by => [qw/mac ssid/], for => 'update',}
          );
      });
    }

    return Status->info(sprintf ' [%s] macsuck - processed %s wireless nodes', $device->ip,
      scalar keys %{$cd11_txrate});
  }
);
