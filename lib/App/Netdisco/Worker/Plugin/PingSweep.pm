...
register_worker(
  ...
  sub {
    my ($job, $workerconf) = @_;
    my $targets = $job->device or return Status->error('missing parameter -d/device with IP prefix');
    ...
    if (!$net or $net->num == 0 or $net->addr eq '0.0.0.0') {
      return Status->error(sprintf 'unable to understand as host, IP, or prefix: %s', $targets);
    }
  ...
  ADDRESS: foreach my $idx (0 .. $net->num()) {
      ...
      # 为响应的主机插入发现任务
      jq_insert([{
        action => 'discover', device => $host, username => ($ENV{USER} || 'netdisco-do'),
      }]);

      ++$job_count;
    }

    return Status->done(sprintf 'Finished ping sweep: queued %s jobs from %s hosts', $job_count, $net->num());
  }
);
