...
register_worker(
  ...
  sub {
    ...
    # 检查设备是否已发现
    return Status->error(sprintf 'Unknown device: %s', ($device || '')) unless $device and $device->in_storage;
...
register_worker(
  ...
  sub {
    ...
    # 检查SNMP错误
    if ($sess->{ErrorNum}) {
      return Status->error(sprintf 'snmp fatal error - %s', $sess->{ErrorStr});
    }
    ...
    return Status->done(
      sprintf 'completed bulkwalk of %s entries from %s for %s',
      (scalar keys %store),
      ($extra || '.1'),
      $device->ip
    );
