...
register_worker(
  ...
  sub {
    ...
    # 检查设备参数
    return Status->error('arpnip failed: unable to interpret device param') unless defined $device;

    # 检查设备是否已发现
    return Status->error("arpnip skipped: $device not yet discovered") unless $device->in_storage;
    ...
    if ($job->port or $job->extra) {
      ...
      return Status->info("arpnip skipped: $device is not arpnipable") unless is_arpnipable_now($device);
    }
