...
register_worker(
  ...
  sub {
    ...
    # 检查设备参数
    return Status->error('macsuck failed: unable to interpret device param') unless defined $device;

    # 检查设备是否已发现
    return Status->error("macsuck skipped: $device not yet discovered") unless $device->in_storage;
    ...
    if ($job->port or $job->extra) {
      ...
      return Status->info("macsuck skipped: $device is not macsuckable") unless is_macsuckable_now($device);
    }
