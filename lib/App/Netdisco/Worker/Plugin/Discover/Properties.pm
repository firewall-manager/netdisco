...
register_worker(
  {phase => 'early', driver => 'snmp', title => 'basic device details (and creation)'},
  sub {
    my ($job, $workerconf) = @_;
    ...
    if ((not $device->vendor or $device->vendor eq 'unknown') and $try_vendor and $try_vendor =~ m/^\.(\d+)/) {
    ...
    if ((not $device->model or $device->model eq 'unknown' or $device->model =~ m/(?:product|enterprise|1\.3\.6\.1)/i)
      and $try_vendor) {
      ...
      if ($object) {
        debug sprintf ' ... found Product "%s" (replaced "%s")', $object->leaf, ($device->model || '');
    ...
    if ($device->model) {
      ...
      if ($model and $model ne $device->model) {
        debug sprintf ' fixing Product Name to be "%s" (was "%s")', $model, $device->model;
        ...
        if ($field eq 'snmp_class') {                              # reject SNMP::Info (it would not be empty)
          ...
          if (not $device->in_storage) {                           # new
            return $job->cancel("discover cancelled: $ip failed to return valid $field")
              if !defined $dirty{$field}
              or not length $dirty{$field};
          }
          elsif ($device->in_storage and ($orig_device->{$field} or length $orig_device->{$field})) {    # existing
            return $job->cancel("rediscover cancelled: $ip failed to return valid $field")
              if !defined $dirty{$field}
              or ($orig_device->{$field} and not $dirty{$field});
          }
        }
      }
    }
    ...
    if ($device->in_storage) {
      ...
      my %ok_fields = map { $_ => 1 } grep {defined} map { $_->{name} } @{setting('custom_fields')->{device} || []};

      # filter custom_fields for current valid fields
      foreach my $field (keys %ok_fields) {
        $ok_fields{$field} = exists $fields->{$field} ? Encode::decode('UTF-8', $fields->{$field}) : undef;
      }
    ...
    schema('netdisco')->txn_do(sub {
      if ($device->serial and setting('delete_duplicate_serials')) {
        my $gone
          = schema('netdisco')
          ->resultset('Device')
          ->search({ip => {'!=' => $device->ip}, serial => $device->serial,})
          ->delete;
...
register_worker(
  {phase => 'early', driver => 'snmp', title => 'cancel if device canonical IP is known to another device'},
  sub {
    my ($job, $workerconf) = @_;
...
register_worker(
  {phase => 'early', driver => 'snmp', title => 'cancel if no valid interfaces found'},
  sub {
    my ($job, $workerconf) = @_;
    ...
    # gather ports
    my $device_ports = {map { ($_->port => $_) } $device->ports(undef, {prefetch => 'properties'})->all};
...
register_worker(
  {phase => 'early', driver => 'snmp', title => 'get device IP aliases and their subnets'},
  sub {
    my ($job, $workerconf) = @_;
    ...
    debug sprintf ' resolving %d aliases with max %d outstanding requests', scalar @aliases,
    ...
    schema('netdisco')->txn_do(sub {
      ...
      debug sprintf ' [%s] device - removed %d aliases', $device->ip, $gone;
      $device->device_ips->populate($resolved_aliases);

      schema('netdisco')->resultset('Subnet')->update_or_create({net => $_, last_discover => \$now}, {for => 'update'})
        for @subnets;

      return Status->info(
        sprintf ' [%s] aliases - added %d new aliases and %d subnets',
        $device->ip,
        scalar @aliases,
        scalar @subnets
      );
...
register_worker(
  {phase => 'early', driver => 'snmp', title => 'get port details'},
  sub {
    my ($job, $workerconf) = @_;
    ...
    my $dev_uptime = ($device->is_pseudo ? $snmp->uptime : $snmp->load_uptime);
    if (!defined $dev_uptime) {
      return Status->info(sprintf ' [%s] interfaces - skipped, no uptime from device', $device->ip);
    }
    ...
    if (defined $snmp->snmpEngineTime) {
      ...
      if ($dev_uptime_wrapped > 0) {
        debug sprintf ' [%s] interfaces - device uptime wrapped %d times - correcting', $device->ip,
          $dev_uptime_wrapped;
  ...
  PORT: foreach my $entry (keys %$interfaces) {
      ...
      if (not $port) {
        debug sprintf ' [%s] interfaces - ignoring %s (no port mapping)', $device->ip, $entry;
        next PORT;
      }

      if (exists $i_ignore->{$entry}) {
        debug sprintf ' [%s] interfaces - ignoring %s (%s) (%s) (SNMP::Info::i_ignore)', $device->ip, $entry, $port,
          ($i_type->{$entry} || '');
        next PORT;
      }
    ...
    if (scalar @{setting('ignore_deviceports')}) {
      ...
      map { push @{$port_map->{$_->port}}, $_ } grep { $_->port } $device->device_ips()->all;

      map { push @{$port_map->{$_->{port}}}, $_ } values %{dclone(\%deviceports || {})};
      ...
      foreach my $map (@{setting('ignore_deviceports')}) {
        ...
        foreach my $key (sort keys %$map) {
          ...
          foreach my $port (sort { sort_port($a, $b) } keys %$port_map) {
            ...
            debug sprintf ' [%s] interfaces - ignoring %s (config:ignore_deviceports)', $device->ip, $port;
  ...
  UPTIME: foreach my $entry (sort keys %$interfaces) {
      ...
      if (not $dev_uptime_wrapped and $lc > ($dev_uptime + 18000)) {
        debug sprintf ' [%s] interfaces - device uptime wrapped (%s) - correcting', $device->ip, $port;
      ...
      if ($device->is_column_changed('uptime') and $lc and $lc < $dev_uptime) {
        ...
        if ($dev_uptime > 30000 and $lc < 30000) {
          ...
          debug sprintf ' [%s] interfaces - correcting LastChange for %s, assuming sysUptime wrap', $device->ip, $port;
    ...
    schema('netdisco')->resultset('DevicePort')->txn_do_locked(
      ACCESS_EXCLUSIVE,
      sub {
        my $coder = JSON::PP->new->utf8(0)->allow_nonref(1)->allow_unknown(1);
        ...
        my %fields
          = map { ($_->{port} => $coder->decode(Encode::encode('UTF-8', $_->{custom_fields} || '{}'))) }
          grep  { exists $deviceports{$_->{port}} }
          $device->ports->search(undef, {columns => [qw/port custom_fields/]})->hri->all;

        my %ok_fields
          = map { $_ => 1 } grep {defined} map { $_->{name} } @{setting('custom_fields')->{device_port} || []};
        ...
        foreach my $port (keys %fields) {
          ...
          foreach my $field (keys %ok_fields) {
            $new_fields{$field}
              = exists $fields{$port}->{$field} ? Encode::decode('UTF-8', $fields{$port}->{$field}) : undef;
          }
        ...
        debug sprintf ' [%s] interfaces - removed %d interfaces', $device->ip, $gone;
        ...
        return Status->info(sprintf ' [%s] interfaces - added %d new interfaces', $device->ip,
          scalar values %deviceports);
      }
    );
  }
);
...
sub _get_ipv4_aliases {
  ...
  foreach my $entry (keys %$ip_index) {
    my $ip   = NetAddr::IP::Lite->new($ip_table->{$entry}) || NetAddr::IP::Lite->new($entry) or next;
    my $addr = $ip->addr;
    ...
    my $subnet = $ip_netmask->{$entry} ? NetAddr::IP::Lite->new($addr, $ip_netmask->{$entry})->network->cidr : undef;

    debug sprintf ' [%s] device - aliased as %s', $device->ip, $addr;
    push @aliases, {alias => $addr, port => $port, subnet => $subnet, dns => undef,};
  }
...
sub _get_ipv6_aliases {
  ...
  foreach my $iid (keys %$ipv6_index) {
    ...
    if (acl_matches($ip, 'group:__LOCAL_ADDRESSES__')) {
      debug sprintf ' [%s] device - skipping alias %s as potentially not unique', $device->ip, $addr;
      next;
    }

    my $port = $interfaces->{$ipv6_index->{$iid}};
    my $subnet
      = $ipv6_pfxlen->{$iid} ? NetAddr::IP::Lite->new($addr . '/' . $ipv6_pfxlen->{$iid})->network->cidr : undef;
