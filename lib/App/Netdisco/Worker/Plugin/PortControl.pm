...
register_worker(
  ...
  sub {
    ...
    # 检查设备参数
    return Status->error('PortControl failed: unable to interpret device param') unless defined $device;

    # 检查设备是否已发现
    return Status->error("PortControl skipped: $device not yet discovered") unless $device->in_storage;
...
sub _action {
  ...
  if (!defined $rv) {
    return Status->error(sprintf "Failed to set [%s] up_admin to [%s] on $device: %s", $pn, $data,
      ($snmp->error || ''));
  }
