...
sub run {
  ...
  $self->job($job);
  $job->device(get_device($job->device)) unless scalar grep { $job->action eq $_ } @{setting('job_targets_prefix')};
  $self->load_workers();
  ...
  my $runner = sub {
    ...
    # 如果我们在测试，回滚所有内容
    my $txn_guard = $ENV{ND2_DB_ROLLBACK} ? schema('netdisco')->storage->txn_scope_guard : undef;
  ...
  my $maxtime
    = ((defined setting($job->action . '_timeout'))
...
sub run_workers {
  ...
  my $set = shift or return $job->add_status(Status->error('missing set param'));
