...
register 'register_worker' => sub {
  ...
  return error "failed to parse action in '$package'" unless $workerconf->{action};
  ...
  $workerconf->{priority}
    ||= (exists $workerconf->{driver} ? (setting('driver_priority')->{$workerconf->{driver}} || 0) : 0);
  ...
  my $worker = sub {
    ...
    debug YELLOW, "\N{RIGHTWARDS BLACK ARROW} worker ", GREY10, $workerconf->{package},
      ($workerconf->{pyworklet} ? (' ' . $workerconf->{pyworklet}) : ''), GREY10, ' p', MAGENTA,
      $workerconf->{priority},
      ($workerconf->{title} ? (GREY10, ' "', BRIGHT_BLUE, $workerconf->{title}, GREY10, '"') : ''), RESET;
    ...
    if ($job->only_namespace and $workerconf->{phase} ne 'check') {
      ...
      if (not(
        $workerconf->{namespace} eq lc($job->only_namespace)

        # 除了discover::properties需要运行，所以对于未知设备是early
        # 阶段，但不是::hooks/early（如果实现）
        or (  ($job->only_namespace ne 'hooks')
          and ($workerconf->{phase} eq 'early')
    ...
    if (ref $job->device) {
      ...
      foreach my $stanza (@userconf) {
        next
          if exists $stanza->{driver}
          and exists $workerconf->{driver}
          and (($stanza->{driver} || '') ne ($workerconf->{driver} || ''));

        # 在这里过滤而不是在Runner中，因为runner不知道命名空间
        next if exists $stanza->{action} and not _find_matchaction($workerconf, lc($stanza->{action}));
  ...
  push @{vars->{'workers'}->{$workerconf->{action}}->{$workerconf->{phase}}->{$workerconf->{namespace}}
...
sub _find_matchaction {
  ...
  foreach my $f (@$action) {
    return true if $f eq $conf->{action} or $f eq "$conf->{action}::$conf->{namespace}";
  }
  return false;
}
