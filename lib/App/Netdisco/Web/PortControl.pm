...
ajax '/ajax/portcontrol' => require_any_role [qw(admin port_control)] => sub {
  send_error('No device/port/field', 400) unless param('device') and (param('port') or param('field'));

  # 构建日志消息
  my $log = sprintf 'd:[%s] p:[%s] f:[%s]. a:[%s] v[%s]', param('device'), (param('port') || ''), param('field'),
  ...
  my $subaction = ($action =~ m/^(?:power|portcontrol)/ ? (param('action') . "-other") : param('value'));
...
ajax '/ajax/userlog' => require_login sub {
  ...
  # 按状态分类作业
  my %status = (
    'done'  => [map { s/\[\]/&lt;empty&gt;/; $_ } map { $_->log } grep { $_->status eq 'done' } grep {defined} @jobs],
    'error' => [map { s/\[\]/&lt;empty&gt;/; $_ } map { $_->log } grep { $_->status eq 'error' } grep {defined} @jobs],
  );
