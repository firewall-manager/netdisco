...
sub add_job {
  ...
  return if ($device and (!$net or $net->num == 0 or $net->addr eq '0.0.0.0'));
  return
    if (($action eq 'discover' or $action eq 'pingsweep')
    and $device
    and (($net->version == 6 and $net->masklen != 128) or ($net->version == 4 and $net->masklen < 22)));
  ...
  my $happy = jq_insert([
    map { {
      action => $action,
      ($_     ? (device => ($action eq 'pingsweep' ? $_ : $_->addr)) : ()), ($port ? (port => $port) : ()),
      ($extra ? (extra  => $extra)                                   : ()),
  ...
  foreach my $h (@hostlist) {
    ...
    my $msg = ($happy ? "Queued job to $action device \%s" : "Failed to queue job to $action device \%s");
...
# 为高优先级和普通优先级作业创建路由
foreach my $action (@{setting('job_prio')->{high}}, @{setting('job_prio')->{normal}}) {

  next if $action and $action =~ m/^hook::/;    # 跳过钩子

  # AJAX管理任务路由
  ajax "/ajax/control/admin/$action" => require_role admin => sub {
    add_job($action, param('device'), param('extra'), param('port')) or send_error('Bad device', 400);
  };
...
ajax qr{/ajax/control/admin/(?:\w+/)?renumber} => require_role admin => sub {
  ...
  send_error('Bad device', 400) if !$device or $device->addr eq '0.0.0.0';

  # 验证新IP
  my $newip = NetAddr::IP->new(param('newip'));
  send_error('Bad new IP', 400) if !$newip or $newip->addr eq '0.0.0.0';
  ...
  # 记录用户日志
  my $msg = ($happy ? 'Queued job to renumber device %s to %s' : 'Failed to queue job to renumber device %s to %s');
...
get "/ajax/content/admin/snapshot_get" => require_role admin => sub {
  ...
  my @rows
    ...
    ->search({ip => $device->addr, -bool => \q{ jsonb_typeof(value) = 'array' },})
    ->hri->all;

  send_error('No snapshot', 400) if 0 == scalar @rows;
