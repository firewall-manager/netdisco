...
hook 'before_template' => sub {
  ...
  return
    ...
    or index(request->path, uri_for('/ajax/content/report/netbios')->path) == 0);
...
get '/ajax/content/report/netbios' => require_login sub {
  ...
  if (defined $domain && !request->is_ajax) {
    ...
    # 处理域名搜索（空白域名转换为空字符串）
    my $search = $domain eq 'blank' ? '' : $domain;
    @results = $rs->search({domain => $search})                   # 按域名过滤
      ->order_by([{-asc => 'domain'}, {-desc => 'time_last'}])    # 按域名升序，最后时间降序排列
      ->hri->all;
  ...
  if (request->is_ajax) {
    ...
    template 'ajax/report/netbios.tt', {results => $json, opt => $domain}, {layout => 'noop'};
  }
  else {
    # 非AJAX请求：返回CSV格式数据
    header('Content-Type' => 'text/comma-separated-values');
    template 'ajax/report/netbios_csv.tt', {results => \@results, opt => $domain}, {layout => 'noop'};
  }
};
