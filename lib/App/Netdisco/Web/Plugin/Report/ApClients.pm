...
get '/ajax/content/report/apclients' => require_login sub {
  ...
  my @results = schema(vars->{'tenant'})->resultset('Device')->search(
    {
      ...
      'ports.port'      =>
        {'-in' => schema(vars->{'tenant'})->resultset('DevicePortWireless')->get_column('port')->as_query},  # 端口必须是无线端口
    }, {
  ...
  if (request->is_ajax) {
    ...
    template 'ajax/report/apclients_csv.tt', {results => \@results}, {layout => 'noop'};
  }
};
