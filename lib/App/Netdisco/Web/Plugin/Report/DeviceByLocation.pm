...
get '/ajax/content/report/devicebylocation' => require_login sub {
  ...
  # 查询所有设备，选择特定字段并按位置排序
  my @results
    = schema(vars->{'tenant'})->resultset('Device')->columns([qw/ ip dns name location vendor model /])    # 选择设备基本信息字段
  ...
  if (request->is_ajax) {
    ...
    template 'ajax/report/devicebylocation_csv.tt', {results => \@results}, {layout => 'noop'};
  }
};
