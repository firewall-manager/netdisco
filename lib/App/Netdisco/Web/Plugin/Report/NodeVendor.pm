...
hook 'before_template' => sub {
  ...
  # 只对节点厂商相关路径生效
  return
    unless (request->path eq uri_for('/report/nodevendor')->path
    or index(request->path, uri_for('/ajax/content/report/nodevendor')->path) == 0);
...
get '/ajax/content/report/nodevendor' => require_login sub {
  ...
  if (request->is_ajax) {
    ...
    template 'ajax/report/nodevendor.tt', {results => $json, opt => $vendor}, {layout => 'noop'};
  }
  else {
    # 非AJAX请求：返回CSV格式数据
    header('Content-Type' => 'text/comma-separated-values');
    template 'ajax/report/nodevendor_csv.tt', {results => \@results, opt => $vendor}, {layout => 'noop'};
  }
};
