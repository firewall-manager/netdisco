...
get '/ajax/content/device/ports' => require_login sub {
  ...
  $prefer = '' unless defined $prefer and $prefer =~ m/^(?:port|name|vlan)$/;
  ...
  if ($f) {
    ...
    if (($prefer eq 'vlan') or (not $prefer and $f =~ m/^\d+$/)) {
      ...
      # 端口或描述匹配
      if (($prefer eq 'port') or not $prefer and $set->search({-or => ['me.port' => $f, 'me.descr' => $f]})->count) {
  ...
  my %port_state = map { $_ => 1 }
    (ref [] eq ref param('port_state') ? @{param('port_state')} : param('port_state') ? param('port_state') : ());
  ...
  if (exists $port_state{free}) {
    if (scalar keys %port_state == 1) {
      ...
      # 包含空闲状态信息
      $set = $set->with_is_free({age_num => (param('age_num') || 3), age_unit => (param('age_unit') || 'months')});
    }
  ...
  if (scalar keys %port_state < 3) {
    ...
    # 添加运行状态过滤条件
    push @combi, {'me.up'       => 'up'}                            if exists $port_state{up};
    push @combi, {'me.up_admin' => 'up', 'me.up' => {'!=' => 'up'}} if exists $port_state{down};
    push @combi, {'me.up_admin' => {'!=' => 'up'}}                  if exists $port_state{shut};
  ...
  my $vlans = $set->search(
    {
      ...
      ? (
        -or => [
          'port_vlans.vlan' => {'<', '1002'},    # 隐藏1002-1005 VLAN
      ...
      select => [
        ...
        {
          array_agg => \q{COALESCE(NULLIF(vlan_entry.description,''), vlan_entry.vlan::text) ORDER BY vlan_entry.vlan},
          -as       => 'vlan_name_set'
        },                                                                                   # VLAN名称集合
      ],
  ...
  if (param('p_vlan_names')) {
    $set = $set->search({},
      {'join' => 'native_vlan', '+select' => [qw/native_vlan.description/], '+as' => [qw/native_vlan_name/],});
  }
  ...
  # 确定IP地址类型
  my $ips_name = ((param('n_ip4') and param('n_ip6')) ? 'ips' : param('n_ip4') ? 'ip4s' : 'ip6s');
  ...
  if (param('c_nodes')) {
    ...
    # 检索无线SSID（如果请求）
    $set = $set->search({}, {prefetch => [{$nodes_name => 'wireless'}]}) if param('n_ssid');

    # 检索NetBIOS信息（如果请求）
    $set = $set->search({}, {prefetch => [{$nodes_name => 'netbios'}]}) if param('n_netbios');

    # 检索厂商信息（如果请求）
    $set = $set->search({}, {prefetch => [{$nodes_name => 'manufacturer'}]}) if param('n_vendor');
  }
  ...
  if (($prefer eq 'vlan') or (not $prefer and $f =~ m/^\d+$/)) {
    if (param('invert')) {
      ...
      @results = grep {
          ...
          and (0 == scalar grep { defined and $_ ne $f } @{$vlans->{$_->port}->{vlan_set}})
      } @results;
    }
    else {
      # 正常过滤：包含匹配的VLAN
      @results = grep {
          ...
          or (scalar grep { defined and $_ eq $f } @{$vlans->{$_->port}->{vlan_set}})
      } @results;
    }
  }
  ...
  if (not param('p_include_hidden')) {
    ...
    # 构建端口映射
    map { push @{$port_map->{$_->port}}, $_ } grep { $_->port } @results;
  ...
  foreach my $port (@results) {
    if ($port->port =~ m/^${subinterfaces_match}$/) {
      ...
      ++$port_has_dot_zero{$parent}
        if $port->port =~ m/\.0$/ and ($port->type and $port->type =~ m/^(?:propVirtual|ieee8023adLag)$/i);
  ...
  foreach my $parent (keys %port_subinterface_count) {
    ...
    $parent_port->{has_only_dot_zero_subinterface} = true
      ...
      and ($parent_port->type and $parent_port->type =~ m/^(?:ethernetCsmacd|ieee8023adLag)$/i);
  ...
  if (param('c_admin') and user_has_role('port_control')) {
    ...
    map {
      $_->{port_acl_name} = (
        $_->{port_acl_service} ||    # 如果服务为真则此权限OK
          port_acl_name($_, $device, logged_in_user)
      )
    } @results;
  }
  ...
  if (request->is_ajax) {
    ...
    template 'ajax/device/ports_csv.tt',
      {results => \@results, nodes => $nodes_name, ips => $ips_name, device => $device, vlans => $vlans,},
      {layout  => 'noop'};
  }
};
