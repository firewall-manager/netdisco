...
ajax '/ajax/data/snmp/typeahead' => require_login sub {
    ...
    my @found = schema(vars->{'tenant'})->resultset('SNMPObject')
      ->search({ -or => [ 'me.oid'  => $term,                                    # 精确OID匹配
                          'me.oid'  => { -like => ($term .'.%') },              # OID前缀匹配
                          -and => [(($mib and $leaf) ? ('me.mib' => $mib, 'me.leaf' => { -ilike => ($leaf .'%') })
                                                     : ('me.leaf' => { -ilike => ('%'. $term .'%') }))] ],  # MIB和叶子匹配
                (($device and $deviceonly) ? ('device_browser.ip' => $device, 'device_browser.value' => { -not => undef }) : ()) },  # 设备专用过滤
...
ajax '/ajax/data/snmp/nodesearch' => require_login sub {
    ...
    if ($partial) {
        ...
        $found = schema(vars->{'tenant'})->resultset('SNMPObject')
          ->search({ -or => [ 'me.oid' => $to_match,                              # 精确OID匹配
                              'me.oid' => { -like => ($to_match .'.%') },        # OID前缀匹配
                              -and => [(($mib and $leaf) ? ('me.mib' => $mib, 'me.leaf' => { -ilike => ($leaf .'%') })
                                                         : ('me.leaf' => { -ilike => ($to_match .'%') }))] ],  # MIB和叶子匹配
                     (($device and $deviceonly) ? ('device_browser.ip' => $device, 'device_browser.value' => { -not => undef }) : ()),  # 设备专用过滤
        ...
        $found = schema(vars->{'tenant'})->resultset('SNMPObject')
          ->search({
            (($mib and $leaf) ? (-and => ['me.mib' => $mib, 'me.leaf' => $leaf])  # MIB和叶子精确匹配
                              : (-or  => ['me.oid' => $to_match, 'me.leaf' => $to_match])),  # OID或叶子精确匹配
            (($device and $deviceonly) ? ('device_browser.ip' => $device, 'device_browser.value' => { -not => undef }) : ()),  # 设备专用过滤
            },{ rows => 1, join => 'device_browser', order_by => 'oid_parts' })->first;
    }
...
ajax '/ajax/content/device/:ip/snmpnode/:oid' => require_login sub {
    ...
    my $value = schema(vars->{'tenant'})->resultset('DeviceBrowser')
      ->search({-and => [-bool => \q{ array_length(oid_parts, 1) IS NOT NULL },  # OID部分不为空
                         -bool => \q{ jsonb_typeof(value) = 'array' }]})      # 值为数组类型
