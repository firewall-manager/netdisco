...
ajax '/ajax/data/device/netmappositions' => require_login sub {
  ...
  my $qdev = schema(vars->{'tenant'})->resultset('Device')->search_for_device($q) or send_error('Bad device', 400);
  ...
  # 验证并过滤真实的主机组和命名主机组
  my @hgrplist = List::MoreUtils::uniq grep { exists setting('host_group_displaynames')->{$_} }
    grep { exists setting('host_groups')->{$_} } grep {defined} @{$hgroup};
...
sub make_node_infostring {
  ...
  # 格式化并返回节点信息字符串
  return sprintf $fmt, $node->ip, ((($node->vendor || '') =~ m/^[aeiou]/i) ? 'an' : 'a'),
    encode_entities(ucfirst($node->vendor || '')),
    (map { defined $_ ? encode_entities($_) : '' }
    map { $node->$_ } (qw/model os os_ver serial uptime_age location contact/)),
...
sub make_link_infostring {
  ...
  # 将端口和描述信息配对
  my @zipped = List::MoreUtils::zip6 @{$link->{left_port}}, @{$link->{left_descr}}, @{$link->{right_port}},
    @{$link->{right_descr}};

  # 格式化并返回链路信息字符串
  return join '<br><br>', map {
    sprintf '<b>%s:%s</b> (%s)<br><b>%s:%s</b> (%s)', encode_entities($left_name), encode_entities($_->[0]),
      encode_entities(($_->[1] || 'no description')), encode_entities($right_name), encode_entities($_->[2]),
      encode_entities(($_->[3] || 'no description'))
  } @zipped;
}
...
get '/ajax/data/device/netmap' => require_login sub {
  ...
  my $links
    = schema(vars->{'tenant'})
    ->resultset('Virtual::DeviceLinks')
    ->search(
    {(($mapshow eq 'depth' and $depth == 1) ? (-or => [{left_ip => $qdev->ip}, {right_ip => $qdev->ip},]) : ())},
  ...
  while (my $link = $links->next) {
    ...
    # 如果链路不对称，这通常是"最佳"链路
    next
      if exists $seen_link{$link->{left_ip} . "\0" . $link->{right_ip}}
    ...
    while ($seen_cloud > 0 and $passes > 0) {
      ...
      foreach my $cip (keys %cloud) {
        foreach my $okip (keys %ok_dev) {
          ...
          # 如果设备间有链路连接，添加到云中
          if (exists $seen_link{$cip . "\0" . $okip} or exists $seen_link{$okip . "\0" . $cip}) {
  ...
  my $devices
    ...
    ->resultset('Device')
    ->search({},
    {'+select' => [\'floor(log(throughput.total))'], '+as' => ['log'], join => 'throughput', distinct => 1,})
    ->with_times->with_custom_fields;

  # 根据VLAN过滤设备（全部或仅邻居）
  if ($vlan) {
    $devices = $devices->search({'port_vlans_filter.vlan' => $vlan}, {join => 'port_vlans_filter'});
  }
...
DEVICE: while (my $device = $devices->next) {
    ...
    next DEVICE if ($device->ip ne $qdev->ip) and ($mapshow ne 'all') and (not $cloud{$device->ip});    # 只显示邻居但没有链路

    # 如果选择了位置，则按位置过滤
    next DEVICE
      if ((scalar @lgrplist)
      and ((!defined $device->location) or (0 == scalar grep { $_ eq $device->location } @lgrplist)));

    # 如果选择了主机组，则使用ACL过滤
    my $first_hgrp = first { acl_matches($device, setting('host_groups')->{$_}) } @hgrplist;
    next DEVICE if ((scalar @hgrplist) and (not $first_hgrp));

    # 重新设置first_hgroup为匹配设备的组（如果有）
    $first_hgrp
      = first { acl_matches($device, setting('host_groups')->{$_}) } keys %{setting('host_group_displaynames') || {}};
    ...
    my %color_lkp = (
      ...
      hgroup => (
        $first_hgrp
        ?                                                      # 主机组颜色
          setting('host_group_displaynames')->{$first_hgrp}
        : 'Other'
      ),
    ...
    # 设置中心节点
    $metadata{'centernode'} = $device->ip if $qdev and $qdev->in_storage and $device->ip eq $qdev->ip;
  }
