...
# 注册管理任务 - 重复设备检测
register_admin_task({tag => 'duplicatedevices', label => 'Duplicate Devices',});
...
ajax '/ajax/content/admin/duplicatedevices' => require_role admin => sub {
  ...
  my @set = schema(vars->{'tenant'})->resultset('Device')->search(
    {
      serial => {
        '-in' => schema(vars->{'tenant'})->resultset('Device')->search(
          {
  ...
  # 渲染重复设备模板
  template 'ajax/admintask/duplicatedevices.tt', {results => \@set}, {layout => undef};
};
