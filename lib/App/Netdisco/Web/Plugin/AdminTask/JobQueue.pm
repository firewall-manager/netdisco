...
# 注册管理任务 - 作业队列管理
register_admin_task({tag => 'jobqueue', label => 'Job Queue',});
...
ajax '/ajax/content/admin/jobqueue' => require_role admin => sub {
  ...
  # 查询后端工作进程信息
  my @backends
    = schema(vars->{'tenant'})->resultset('DeviceSkip')->search({device => '255.255.255.255'})->hri->all; # 特殊设备IP表示后端进程
  ...
  # 查询各种作业状态统计
  my $jq_locked
    = schema(vars->{'tenant'})->resultset('Admin')->search({status => 'queued', backend => {'!=' => undef}})->count()
    ;                                                                                                     # 正在运行的作业

  my $jq_backlog
    = schema(vars->{'tenant'})->resultset('Admin')->search({status => 'queued', backend => undef})->count();  # 等待队列中的作业

  my $jq_done = schema(vars->{'tenant'})->resultset('Admin')->search({status => 'done'})->count();            # 已完成的作业

  my $jq_errored = schema(vars->{'tenant'})->resultset('Admin')->search({status => 'error'})->count();        # 出错的作业
