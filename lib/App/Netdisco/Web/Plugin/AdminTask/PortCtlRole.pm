...
# 注册管理任务 - 端口控制角色管理
register_admin_task({tag => "portctlrole", label => "Port Control Roles"});

# 端口控制角色内容路由 - 显示所有端口控制角色
ajax '/ajax/content/admin/portctlrole' => require_role admin => sub {

  # 获取所有端口控制角色名称
  my @roles = schema(vars->{'tenant'})->resultset('PortCtlRole')->role_names;

  # 渲染端口控制角色模板，按名称排序
  template 'ajax/admintask/portctlrole.tt', {results => [sort @roles],}, {layout => undef};
};
...
ajax '/ajax/control/admin/portctlrole/add' => require_role admin => sub {
  ...
  send_error('Bad Request', 400)
    if schema(vars->{'tenant'})->resultset('PortCtlRole')->search({role_name => $role})->count();
...
ajax '/ajax/control/admin/portctlrole/delete' => require_role admin => sub {
  ...
  # 在事务中删除角色及其相关数据
  schema(vars->{'tenant'})->txn_do(sub {
    my $rows = schema(vars->{'tenant'})->resultset('PortCtlRole')->search({role_name => $role}) or return;  # 如果角色不存在则返回

    # 删除相关的访问控制列表
    schema(vars->{'tenant'})
      ->resultset('AccessControlList')
      ->search({id => {-in => [$rows->device_acls]}})
      ->delete;                                                                                             # 删除设备ACL
    schema(vars->{'tenant'})
      ->resultset('AccessControlList')
      ->search({id => {-in => [$rows->port_acls]}})
      ->delete;                                                                                             # 删除端口ACL

    $rows->delete;                                                                                          # 删除角色记录

    # 更新使用该角色的用户
    schema(vars->{'tenant'})->resultset('User')->search({portctl_role => $role})->update({

      # 如果角色在配置中存在，则保留；否则清除端口控制权限
      ((exists config->{'portctl_by_role_shadow'}->{$role}) ? () : (portctl_role => undef, port_control => \'false')),
    });
  });
};
...
ajax '/ajax/control/admin/portctlrole/update' => require_role admin => sub {
  ...
  schema(vars->{'tenant'})->txn_do(sub {
    ...
    # 更新角色表中的角色名称
    schema(vars->{'tenant'})->resultset('PortCtlRole')->search({role_name => $old_role})->update({role_name => $role});

    # 更新用户表中使用该角色的用户
    schema(vars->{'tenant'})->resultset('User')->search({portctl_role => $old_role})->update({portctl_role => $role});
  });
};
