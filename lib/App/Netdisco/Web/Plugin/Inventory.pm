...
# 注册导航栏项目 - 库存管理页面入口
register_navbar_item({tag => 'inventory', path => '/inventory', label => 'Inventory',});
...
get '/inventory' => require_login sub {
  ...
  my %release_version_map = (
    map {
      (join '', map { sprintf '%05s', $_ } split m/(\D)/, ($_->{os_ver} || '')) => $_
    } $releases->hri->all
  );
  ...
  my %release_totals = map { $_ => {rows => scalar @{$release_map{$_}}, count => 0} } keys %release_map;

  # 累加每个操作系统类型的设备数量
  foreach my $r (keys %release_totals) {
    map { $release_totals{$r}->{count} += $_->{count} } @{$release_map{$r}};
  }
  ...
  # 计算每个厂商的统计信息（行数和设备总数）
  my %platform_totals = map { $_ => {rows => scalar @{$platform_map{$_}}, count => 0} } keys %platform_map;

  # 累加每个厂商的设备数量
  foreach my $r (keys %platform_totals) {
    map { $platform_totals{$r}->{count} += $_->{count} } @{$platform_map{$r}};
  }
