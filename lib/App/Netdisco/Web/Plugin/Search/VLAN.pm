...
register_search_tab({
  ...
  provides_csv   => 1,
  api_endpoint   => 1,
  api_parameters => [q => {description => 'VLAN name or number', required => 1,},],
});
...
get '/ajax/content/search/vlan' => require_login sub {
  ...
  if ($q =~ m/^\d+$/) {
    ...
    # 数字查询：按VLAN编号搜索
    $rs = schema(vars->{'tenant'})->resultset('Device')->carrying_vlan({vlan => $q});
  }
  else {
    # 文本查询：按VLAN名称搜索
    $rs = schema(vars->{'tenant'})->resultset('Device')->carrying_vlan_name({name => $q});
  }
