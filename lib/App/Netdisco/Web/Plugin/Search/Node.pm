...
register_search_tab({
  ...
  api_parameters => [
    q => {
      description =>
        'MAC Address or IP Address or Hostname (without Domain Suffix) of a Node (supports SQL or "*" wildcards)',
      required => 1,
    },
    partial => {
    ...
    deviceports =>
      {description => 'MAC Address search will include Device Port MACs', type => 'boolean', default => 'true',},
    show_vendor => {description => 'Include interface Vendor in results', type => 'boolean', default => 'false',},
    archived    => {description => 'Include archived records in results', type => 'boolean', default => 'false',},
    daterange   => {
      description => 'Date Range in format "YYYY-MM-DD to YYYY-MM-DD"',
      default     => ('1970-01-01 to ' . strftime('%Y-%m-%d', gmtime)),
    },
    age_invert => {description => 'Results should NOT be within daterange', type => 'boolean', default => 'false',},
...
get '/ajax/content/search/node' => require_login sub {
  ...
  undef $mac
    ...
    $mac and $mac->as_ieee and (
      ($mac->as_ieee eq '00:00:00:00:00:00')    # 无效的MAC地址
  ...
  my @where_mac = (
    $using_wildcards
    ? \['me.mac::text ILIKE ?', $likeval]    # 使用通配符搜索
    : ((!defined $mac or $mac->errstr) ? \'0=1' : ('me.mac' => $mac->as_ieee))
  );                                         # 精确MAC匹配

  # 查询节点目击记录
  my $sightings = schema(vars->{'tenant'})->resultset('Node')->search(
    {-and => [@where_mac, @active, @times]},
    {
  ...
  # 查询节点IP记录
  my $ips = schema(vars->{'tenant'})->resultset('NodeIp')->search(
    {-and => [@where_mac, @active, @times]},
    {
  ...
  # 查询NetBIOS记录
  my $netbios = schema(vars->{'tenant'})->resultset('NodeNbt')->search(
    {-and => [@where_mac, @active, @times]},
    {
      order_by   => {'-desc' => 'time_last'},
      '+columns' => [
  ...
  my $wireless = schema(vars->{'tenant'})->resultset('NodeWireless')->search(
    {-and => [@where_mac, @wifitimes]},
    {
      order_by   => {'-desc' => 'time_last'},
      '+columns' => [
        'manufacturer.company', 'manufacturer.abbrev',
        {time_last_stamp => \"to_char(time_last, 'YYYY-MM-DD HH24:MI')"}
      ],
      join => 'manufacturer'
    }
  ...
  if ($sightings->has_rows or $ips->has_rows or $netbios->has_rows) {
    my $ports
      = param('deviceports') ? $rs_dp->search({-and => [@where_mac]}, {order_by => {'-desc' => 'creation'}}) : undef;
    ...
    my $ports
      = param('deviceports')
      ? $rs_dp->search({-and => [@where_mac, @porttimes]}, {order_by => {'-desc' => 'creation'}})
      : undef;

    if (defined $ports and $ports->has_rows) {
      return template 'ajax/search/node_by_mac.tt',
        {ips    => $ips, sightings => $sightings, ports => $ports, wireless => $wireless, netbios => $netbios,},
        {layout => 'noop'};
    }
  }
  ...
  # 尝试其他搜索方法
  my $have_rows = 0;
  my $set       = schema(vars->{'tenant'})->resultset('NodeNbt')->search_by_name({nbname => $likeval, @active, @times});
  ...
  unless ($have_rows) {
    ...
    # 检查是否为IP地址格式
    if ($node =~ m{^(?:$RE{net}{IPv4}|$RE{net}{IPv6})(?:/\d+)?$}i and my $ip = NetAddr::IP::Lite->new($node)) {

      # search_by_ip()会提取CIDR表示法（如果需要）
      $set = schema(vars->{'tenant'})->resultset('NodeIp')->search_by_ip({ip => $ip, @active, @times})->with_router;
      ++$have_rows if $set->has_rows;
    }
    else {
      # 尝试DNS名称搜索
      $set = schema(vars->{'tenant'})->resultset('NodeIp')->search_by_dns({
        (
      ...
      if (param('show_vendor') and not $have_rows) {
        $set = schema(vars->{'tenant'})->resultset('NodeIp')->with_times->search(
