...
get '/ajax/content/search/port' => require_login sub {
    ...
    if ($q =~ m/^[0-9]+$/ and $q < 4096) {
        ...
        undef $mac if
          ($mac and $mac->as_ieee
          and (($mac->as_ieee eq '00:00:00:00:00:00')  # 无效MAC地址
        ...
        $rs = schema(vars->{'tenant'})->resultset('DevicePort')
            ...
            {   '+columns' => [qw/ device.dns device.name /, {vlan_agg => q{array_to_string(array_agg(port_vlans.vlan), ', ')}} ],  # VLAN聚合
                join       => [qw/ properties port_vlans device /],
