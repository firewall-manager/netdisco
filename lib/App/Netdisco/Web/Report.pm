...
get '/report/*' => require_login sub {
  ...
  # 根据报告类型获取相应的数据列表
  if ($tag eq 'netbios') {
    $domain_list = [schema(vars->{'tenant'})->resultset('NodeNbt')->get_distinct_col('domain')];
  }
  elsif ($tag eq 'moduleinventory') {
    $class_list = [schema(vars->{'tenant'})->resultset('DeviceModule')->get_distinct_col('class')];
  }
  elsif ($tag eq 'portssid') {
    $ssid_list = [schema(vars->{'tenant'})->resultset('DevicePortSsid')->get_distinct_col('ssid')];
  }
  elsif ($tag eq 'nodesdiscovered') {
    $type_list = [schema(vars->{'tenant'})->resultset('DevicePort')->get_distinct_col('remote_type')];
  }
