...
sub get_user_details {
  ...
  my $database = schema($settings->{schema_name}) or die "No database connection";
  ...
  if (
    not $user
    and (setting('no_auth')
      or (not setting('validate_remote_user') and (setting('trust_remote_user') or setting('trust_x_remote_user'))))
  ) {

    $user = $database->resultset($users_table)->new_result({username => $username});
  }
...
sub get_user_roles {
  ...
  my $api_requires_key
    = (setting('trust_remote_user') or setting('trust_x_remote_user') or setting('no_auth')) eq '1' ? 'false' : 'true';

  return [
    try {
      $user->$roles->search({},
        {bind => [$api_requires_key, setting('api_token_lifetime'), $api_requires_key, setting('api_token_lifetime'),]})
        ->get_column($role_column)
        ->all;
    }
  ];
}
...
sub _ldap_search {
  ...
  foreach my $server (@{$conf->{servers}}) {
    ...
    # 执行搜索
    $msg = $ldap->search(base => $conf->{base}, filter => "($filter)", attrs => $attrs,);
...
sub match_with_radius {
  ...
  my $servers = (ref [] eq ref $conf->{'server'} ? $conf->{'server'} : [$conf->{'server'}]);

  # 创建RADIUS客户端
  my $radius
    = Authen::Radius->new(NodeList => $servers, Secret => $conf->{'secret'}, TimeOut => $conf->{'timeout'} || 15,);

  # 加载RADIUS字典
  my $dict_dir
    = Path::Class::Dir->new(dist_dir('App-Netdisco'))
    ->subdir('contrib')
    ->subdir('raddb')
    ->file('dictionary')
    ->stringify;
  ...
  # 添加用户属性
  $radius->add_attributes({Name => 'User-Name', Value => $user}, {Name => 'User-Password', Value => $pass});
