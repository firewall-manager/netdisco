...
ajax '/ajax/data/queue/typeahead/backend' => require_role admin => sub {
  ...
  my @backends
    = grep                          { $q ? m/$q/ : true }
    List::MoreUtils::uniq sort grep {defined}
...
ajax '/ajax/data/queue/typeahead/action' => require_role admin => sub {
  ...
  foreach my $plugin (@user_plugins, @core_plugins) {
    ...
    $plugin = 'App::Netdisco::Worker::Plugin::' . $plugin if $plugin !~ m/^\+/;
    $plugin =~ s/^\+//;
  ...
  # 添加数据库中的动作
  push @actions, schema(vars->{'tenant'})->resultset('Admin')->get_distinct_col('action');

  my $q = quotemeta(param('query') || param('term') || param('action'));

  content_type 'application/json';
  to_json [grep { $q ? m/^$q/ : true } grep {defined} List::MoreUtils::uniq sort @actions];
};
...
ajax '/ajax/data/queue/typeahead/status' => require_role admin => sub {
  ...
  # 预定义的作业状态列表
  my @actions = grep { $q ? m/^$q/ : true } qw(Queued Running Done Info Deferred Error);
...
ajax '/ajax/data/devicename/typeahead' => require_login sub {
  ...
  my $set
    ...
    ->resultset('Device')
    ->search_fuzzy($q)
    ->search(undef, {rows => setting('max_typeahead_rows')});
...
ajax '/ajax/data/deviceip/typeahead' => require_login sub {
  ...
  while (my $d = $set->next) {
    ...
    if ($d->dns or $d->name) {
      $label = sprintf '%s (%s)', ($d->dns || $d->name), $d->ip;
    }
...
ajax '/ajax/data/devices/typeahead' => require_login sub {
  ...
  # TODO 从数据库添加条目
  my @host_groups = sort { $a cmp $b } grep { $_ !~ m/^synthesized_group_/ } keys %{setting('host_groups')};
  ...
  if (scalar grep { $_ =~ m/\Q$q\E/i } @host_groups) {
    push @data, map { 'group:' . $_ } grep { $_ =~ m/\Q$q\E/i } @host_groups;
  }
...
ajax '/ajax/data/port/typeahead' => require_login sub {
  ...
  # 查找设备
  my $device = schema(vars->{'tenant'})->resultset('Device')->find({ip => $dev});
  ...
  # 如果提供了端口参数，则进行模糊搜索
  $set = $set->search({port => {-ilike => "\%$port\%"}}) if $port;

  # 构建结果，按端口排序
  my $results = [map { {label => (sprintf "%s (%s)", $_->port, ($_->name || '')), value => $_->port} }
...
ajax '/ajax/data/subnet/typeahead' => require_login sub {
  ...
  # 搜索子网
  my $nets = schema(vars->{'tenant'})->resultset('Subnet')->search({'me.net::text' => {'-ilike' => $q}},
