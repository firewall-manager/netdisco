...
sub new {
  ...
  my $self = bless {interval => $interval, timeout => $timeout, %args}, $class;

  Scalar::Util::weaken(my $wself = $self);

  socket my $fh4, AF_INET, Socket::SOCK_DGRAM(), 0 or Carp::croak "Unable to create socket : $!";
...
sub nbtstat {
  ...
  my $request = {host => $host, results => {}, cb => $cb, destination => scalar sockaddr_in($port, $ip),};
...
sub _on_read {
  ...
  ($resp) = $resp =~ /^(.*)$/s if AnyEvent::TAINT && $self->{untaint};
...
sub _store_result {
  ...
  if ($status eq 'OK' && length($resp) > 56) {
    ...
    $mac_address = join "-", map { sprintf "%02X", $_ } unpack("C*", substr($name_data, 18 * $num_names, 6));
    $results     = {'status' => 'OK', 'names' => \@rr, 'mac_address' => $mac_address};
  }
  elsif ($status eq 'OK') {
...
sub _send_request {
  ...
  send $fh, $msg, 0, $request->{destination} or $self->_store_result($request, 'ERROR');
