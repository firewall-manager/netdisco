...
BEGIN {
  if (setting('include_paths') and ref [] eq ref setting('include_paths')) {
    ...
    # 将有用的位置添加到@INC中
    push @{setting('include_paths')}, dir(($ENV{NETDISCO_HOME} || $ENV{HOME}), 'nd-site-local', 'lib')->stringify
...
if (ref {} eq ref setting('database')) {
  ...
  setting('database')->{host} = ($ENV{NETDISCO_DB_HOST} || $ENV{PGHOST} || setting('database')->{host});

  my $portnum = ($ENV{NETDISCO_DB_PORT} || $ENV{PGPORT});
  setting('database')->{host} .= (';port=' . $portnum) if (setting('database')->{host} and $portnum);

  # 曾经我们要求用户添加port=
  setting('database')->{host} =~ s/port=port=/port=/ if $portnum;

  setting('database')->{user} = ($ENV{NETDISCO_DB_USER} || $ENV{PGUSER} || setting('database')->{user});

  setting('database')->{pass} = ($ENV{NETDISCO_DB_PASS} || $ENV{PGPASSWORD} || setting('database')->{pass});
  ...
  setting('plugins')->{DBIC}->{'default'} ||= {
    ...
    user         => $user,
    password     => $pass,
    options      => {AutoCommit => 1, RaiseError => 1, auto_savepoint => 1, pg_enable_utf8 => 1,},
  ...
  # 允许覆盖默认租户
  setting('plugins')->{DBIC}->{'default'} = setting('plugins')->{DBIC}->{$ENV{NETDISCO_DB_TENANT}}
  ...
  foreach my $c (@{setting('external_databases')}) {
    ...
    setting('plugins')->{DBIC}->{$schema}->{schema_class} ||= 'App::Netdisco::GenericDB';
  }
}
...
# 从环境变量覆盖（用于docker）
config->{'community'}
  = ($ENV{NETDISCO_RO_COMMUNITY} ? [split ',', $ENV{NETDISCO_RO_COMMUNITY}] : config->{'community'});
config->{'community_rw'}
  = ($ENV{NETDISCO_RW_COMMUNITY} ? [split ',', $ENV{NETDISCO_RW_COMMUNITY}] : config->{'community_rw'});
...
config->{'device_auth'} = [App::Netdisco::Util::DeviceAuth::fixup_device_auth()];
...
$ENV{'PERL_ANYEVENT_MAX_OUTSTANDING_DNS'} = setting('dns')->{max_outstanding} || 50;
...
setting('dns')->{'ETCHOSTS'} = {};
{
  ...
  setting('dns')->{'ETCHOSTS'}->{$_}
    = [map { [$_ ? (format_address $_->[0]) : ''] } @{$AnyEvent::DNS::EtcHosts::HOSTS{$_}}]
...
config->{'bulkwalk_off'} = true if (exists $ENV{NETDISCO_SNMP_BULKWALK_OFF} and $ENV{NETDISCO_SNMP_BULKWALK_OFF});

# 检查用户的port_control_reasons

config->{'port_control_reasons'} = config->{'port_control_reasons'} || config->{'system_port_control_reasons'};

# 用于管理数据库portctl_roles

config->{'portctl_by_role_shadow'} = dclone(setting('portctl_by_role') || {});

# 将domain_suffix从标量或列表转换为正则表达式

config->{'domain_suffix'} = [setting('domain_suffix')] if ref [] ne ref setting('domain_suffix');
...
if (q{} eq ref setting('expire_devices')) {
  config->{'expire_devices'} = {'group:__ANY__' => setting('expire_devices')};
}

# 将tacacs从单个值转换为列表

if (ref {} eq ref setting('tacacs') and exists setting('tacacs')->{'key'}) {
...
config->{'field_protection'} = config->{'snmp_field_protection'} if exists config->{'snmp_field_protection'};
...
config->{'devport_vlan_limit'} = config->{'deviceport_vlan_membership_threshold'}
...
config->{'schedule'} = config->{'housekeeping'} if setting('housekeeping') and not setting('schedule');
delete config->{'housekeeping'};

# 以前有不同类型的工作进程
if (exists setting('workers')->{interactives} or exists setting('workers')->{pollers}) {

  setting('workers')->{tasks} ||= (setting('workers')->{pollers} || 0) + (setting('workers')->{interactives} || 0);
...
# （例如在swagger_path()内部时）
config->{url_base} = URI::Based->new((config->{path} eq '/') ? '' : config->{path});
config->{api_base} = config->{url_base}->with('/api/v1')->path;
...
foreach my $field (@{setting('custom_fields')->{'device'}}) {
  ...
  push @new_hooks, {
    ...
    with  => {
      ...
      cmd => (
        sprintf q![%% ndo %%] show -d '[%% ip %%]' -e %s --quiet!

          # 这个jq将：将null提升为[]，将裸字符串提升为["str"]，将对象折叠为列表
          . q! | jq -cjM '. // [] | if type=="string" then [.] else . end | [ .[] ] | sort'!

          # 将JSON输出发送到设备custom_field（内联操作）
          . q! | [%% ndo %%] %s --enqueue -d '[%% ip %%]' -e '@-' --quiet!, $field->{'snmp_object'},
        ('cf_' . $field->{'name'})
      ),
    },
    filter => {no => $field->{'no'}, only => $field->{'only'},},
  };
  push @new_dcf, $field;
}
...
foreach my $action (qw(macsuck arpnip)) {
  push @new_hooks, {
    type  => 'exec',
    event => 'new_device',
    with  => {cmd => (sprintf q![%% ndo %%] %s --enqueue -d '[%% ip %%]' --quiet!, $action)}
    };
}
